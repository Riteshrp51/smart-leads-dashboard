...
import { User, Mail, ShieldAlert, ArrowLeft, Edit3 } from 'lucide-react';
...
const EditLead: React.FC = () => {
  ...
  // Fetch lead data by ID on mount
  useEffect(() => {
    if (id) {
      fetchLeadById(id);
    }
  ...
  // Reset/populate form once selectedLead is loaded
  useEffect(() => {
    if (selectedLead) {
      reset({
        ...
        <main className="flex-1 p-6 md:p-8 space-y-6 max-w-3xl mx-auto w-full">
          ...
          {loading ? (
            ...
            <div className="bg-white dark:bg-[#111827]/40 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-12 text-center backdrop-blur-xl">
              <p className="text-slate-500 dark:text-slate-400 text-sm font-semibold">Lead not found or unauthorized.</p>
            </div>
          ) : (
            /* Form Card */
            <div className="bg-white dark:bg-[#111827]/40 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-6 md:p-8 backdrop-blur-xl shadow-md dark:shadow-2xl relative">
              <div className="flex items-center gap-3 mb-6 pb-6 border-b border-slate-200 dark:border-b-slate-800/80">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Edit Lead: {selectedLead.name}</h2>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Update profile info, change status, or assign communication channels.
                  </p>
                </div>
              </div>
